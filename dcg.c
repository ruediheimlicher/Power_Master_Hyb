...
<html><head><script src="safari-extension://com.wondershare.safari.itubestudio-YZC2T44ZDX/24636cf9/analyster/common.js"></script><script src="safari-extension://com.wondershare.safari.itubestudio-YZC2T44ZDX/24636cf9/Core.js"></script><script src="safari-extension://com.wondershare.safari.itubestudio-YZC2T44ZDX/24636cf9/extensionconfig.js"></script><style type="text/css"></style></head><body><pre style="word-wrap: break-word; white-space: pre-wrap;">/*
...
double ArbMinVoltage = 0.0; // to calculate the voltage range for Arbitrary Mode with respect to relais switching; internally used only
...
int uart_putchar(char c, FILE* stream __attribute__((unused)))
{
   ...
   {
      c = '\r';
      while (1 != setUartTxData((uint8_t*)&c, 1, 0));
      c = '\n';
   }
   while(1 != setUartTxData((uint8_t*)&c, 1, 0));
   return 0;
...
void jobGetValues(void)
{
   ...
   else
   {
      ...
      {
         Values.u16[0] = ValuesLow.u16[0] = GetADC(2); // not implemented in 12-bit mode
      }
...
void InitScales(void)
{
   ...
   {
      Ufac = 2 * Params.RefVoltage;   // LTC1655 intern 2fache Verstärkung
      tmpDACMax = 0x10000;
   }
   else
   {
   ...
   DACMax = tmpDACMax - 1; // für SetLevelDAC
...
void SetLevelDAC(void)
{
   ...
   if ( ArbActive == 0 )
   {
      ...
      // Berechnung des DAC-Wertes für Spannung für Ripple-Low U = (Uo* (100-RippleMod))/100
   ...
   else
   {
      ...
      memcpy_P(&ArbArrayV_Ptr, &ArbArrayV[ArbSelect], sizeof(const PROGMEM double *));
      memcpy_P(&ArbArrayT_Ptr, &ArbArrayT[ArbSelect], sizeof(const PROGMEM uint16_t *));
      ...
      if (LockRangeU == 255)
      {
         ...
         do      // for all voltage values of the array:
         {
            ...
            //      - find the lowest voltage for correct calculation of relais switching
            ...
            //          if ( tmpArbV > Params.MaxVoltage[0] )
...
void jobFaultCheck(void)
{
   ...
   tmpVolt = GetPowerIn() - 2;
   if (xMeanVoltage > tmpVolt)
   {
      if (!Status.OverVolt)
      {
         DPRINT(PSTR("%-10lu Vdc: %.2f, Vin: %.2f\n"), GetTicker(), xMeanVoltage, tmpVolt);
   ...
   if (Flags.RelaisState == 2)
   {
      // nur Prüfen, wenn Relais eingeschaltet sind
      Status.FuseBlown = (tmpVolt < 5.0);
   }
   else
   {
      Status.FuseBlown = 0;
...
void jobSwitchRelay(void)
{
   ...
   /*    if (xMeanVoltage > Params.RelayVoltage - RippleVoltage)
    {
    Flags.PwrInRange = 1;
    }
    // Philosphy: kick down if regular (max<=>min) Voltage fits below RelayVoltage
    else */ if (xMeanVoltage + 0.5 < Params.RelayVoltage - ( ArbActive ? (wVoltage - ArbMinVoltage) : RippleVoltage ) )  // kick-down to lower input voltage in overcurrent and ripple mode 0.5V hysteresis
    {
       // depeding on mode: minimum voltage of ripple or arbitrary
       Flags.PwrInRange = 0;
   ...
   switch(Flags.RelaisState)
   {
      case 0: // off
         if (!Status.OverVolt && !Status.OverTemp)
         {
            if(Flags.PwrInRange)
            {
               DPRINT(PSTR("%-10lu Relais 1: on\n"), GetTicker());
               PORTB |= (1<<PB2);  // Relais 1 ein
            }
            else
            {
               DPRINT(PSTR("%-10lu Relais 2: on\n"), GetTicker());
               PORTB |= (1<<PB3);  // Relais 2 ein
            }
            RelayTimer = 2; // 8ms
            Flags.RelaisState = 1;
         }
      ...
      case 2: // on
         if (Status.OverVolt || Status.OverTemp)
         {
            ...
            {
               DPRINT(PSTR("%-10lu Relais 1,2: off\n"), GetTicker());
            }
            else if (PORTB & (1<<PB2))
            {
               DPRINT(PSTR("%-10lu Relais 1: off\n"), GetTicker());
            }
            else if(PORTB & (1<<PB3))
            {
               DPRINT(PSTR("%-10lu Relais 2: off\n"), GetTicker());
            }
#endif
            PORTB &= ~(1<<PB2); // Relais 1 aus
            PORTB &= ~(1<<PB3); // Relais 2 aus
            RelayTimer = 2; // 8ms
            Flags.RelaisState = 1;
         }
...
int main(void)
{
   ...
   {
      LCDWrite_P(0, 0, strlen_P(InitEEPStr_P), InitEEPStr_P);
   ...
   // UART initialisieren
   initUartUBRR(Params.SerBaudReg);
   ...
   LCDWrite_P(0, 0, strlen_P(VersStrShort_P), VersStrShort_P);
   ...
   DCAmpMod = 1;
   wVoltage = 0;
   wCurrent = 0;               // Für den Einschaltvorgang mit Q3 (PC7-->1) Sollstrom auf Null einstellen
   ArbActive = 0;
   
   SetLevelDAC();              // Anfangswerte
   wait_us(4000);              // warten, daß die ersten DAC Werte, Strom und Spannung = 0 ausgegeben wurden
   
   PORTC |= (1<<PC7);          // Outputenable == Ausgang ein
   wait_us(4000);              // warten, daß die Regelschleife eingeschwungen ist
