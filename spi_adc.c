...
{
   //SPCR0=0;
 //  SPI_PORT |= (1<<(SPI_CS_ADC_PIN));    //setbitHigh CS   Pin
...
void MCP3208_spi_Init(void)
{
   ...
   SPI_DDR = (1<<SPI_MOSI_PIN)|(1<<SPI_SCK_PIN)|(1<<SPI_CS_ADC_PIN);  // set DD_SS to output
...
{
   //OSZI_A_LO ;
   unsigned char  tempHigh,tempLow,tempADtype,dummyData =0;
   
   SPI_PORT &= ~(1<<(SPI_CS_ADC_PIN));     //setbitLow CS  Pin
   _delay_us(ADC_DELAY);
